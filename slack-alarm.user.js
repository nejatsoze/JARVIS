...
(function () {
    ...
    // ============ HEDEFLER ============
    // I ve II aynı listeyi izler; kademeler sadece ne kadar çalacağını belirler.
    const HEDEFLER = [
        'admin', 'Anderson', 'Joseph', 'Minerva', 'çekim-talepleri',
        'finansal-sorunlar',
        'canlıdestek-ortak',
        'bonus-kontrol',
        'çevrim-kontrol',
    ...
    // calma: yeni bir okunmamış mesaj başına en fazla kaç kez çalacağı.
    const SEVIYELER = [
        { kisa: '0', ad: 'Kapalı', aciklama: 'Hiç alarm çalmaz', renk: '#94a3b8', liste: [], calma: 0 },
        { kisa: 'I', ad: 'Kısa', aciklama: 'Her yeni mesajda iki kez çalar', renk: '#f0a132', liste: HEDEFLER, calma: 2 },
        { kisa: 'II', ad: 'Israrlı', aciklama: 'Mesaj okunana kadar çalmaya devam eder', renk: '#e4574c', liste: HEDEFLER, calma: Infinity },
    ];

    // Alarmın kaç saniyede bir tekrar çalacağı (II'de okunana kadar, I'de ikinci çalış).
    ...
    // Bir "olay" = bir hedefin okunmamışa dönmesi. I'de olay başına 2 çalış,
    // II'de okunana kadar; yeni bir hedef okunmamışa dönerse sayaç sıfırlanır.
    let calinan = 0;
    let sonUnread = new Set();

    function cal() {
        // Kademe başka bir sekmede değişmiş olabilir: çalmadan önce kayda bak.
        if (seviyeOku() !== aktifSeviye) { seviyeAyarla(seviyeOku(), false); return; }
        const limit = SEVIYELER[aktifSeviye].calma;
        if (unreadSayisi() === 0 || calinan >= limit) { timeriDurdur(); uiDurumGuncelle(unreadSayisi()); return; }
        alarmSesiCal();
        calinan += 1;
        if (calinan >= limit) timeriDurdur();
        uiDurumGuncelle(unreadSayisi());
    }

    function tekrarTimeriGuncelle() {
        const simdiki = new Set([...durum].filter(([, d]) => d.unread).map(([k]) => k));
        const yeniVar = [...simdiki].some((k) => !sonUnread.has(k));
        sonUnread = simdiki;
        ...
        if (simdiki.size === 0) {
            ...
            timeriDurdur();
            calinan = 0;
            cal(); // ilk tetiklemede hemen çal
            if (calinan < SEVIYELER[aktifSeviye].calma) {
                tekrarTimer = setInterval(cal, TEKRAR_ARALIGI_SN * 1000);
            }
        }

        uiDurumGuncelle(simdiki.size);
    }
    ...
    function seviyeAyarla(yeni, sesTesti) {
        aktifSeviye = yeni;
        if (localStorage.getItem(KAYIT_SEVIYE) !== String(yeni)) localStorage.setItem(KAYIT_SEVIYE, String(yeni));
        durum.clear();
        sonUnread = new Set();
        calinan = 0;
        timeriDurdur();
    ...
    // Başka bir Slack sekmesinde kademe değişince bu sekme de hemen uysun
    // (önceden 0 seçilse bile diğer sekmedeki kopya çalmaya devam ediyordu).
    window.addEventListener('storage', (e) => {
        if (e.key === KAYIT_SEVIYE && seviyeOku() !== aktifSeviye) seviyeAyarla(seviyeOku(), false);
    });
    ...
    function uiDurumGuncelle(sayi) {
        ...
        const seviye = SEVIYELER[aktifSeviye];
        ui.kapsul.classList.toggle('calar', sayi > 0 && !!tekrarTimer);
        ...
        if (sesKilitli) {
            ...
            ui.metin.textContent = `${sayi} kanal bekliyor` + (tekrarTimer ? '' : ' · sustu') + (belirsiz > 0 ? ` (${belirsiz} belirsiz)` : '');
        } else if (belirsiz > 0) {
