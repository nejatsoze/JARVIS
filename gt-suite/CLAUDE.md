...
- Butonlar (kullanıcının onayladığı "Minimal + adacık" tasarımı): her zaman
  `GT.ui.button` / `.gt-btn`, kendi buton CSS'i yazma. Beyaz buton, ince çizgi,
  hafif gölge; renk başta küçük bir noktada: `danger` kırmızı nokta + kırmızı
  metin (red, oturum sonlandır), `warn` turuncu nokta (bakiye, CHK), `success`
  yeşil / `muted` gri nokta (OTORED açık/kapalı). Dolu birincil: `primary`
  yeşil (ONAY), `blue` mavi (Bilgileri göster). İlişkili butonlar `.gt-group`
