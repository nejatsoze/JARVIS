...
/* Stiller bir kayıtta tutulur ve HER ZAMAN o anki belgeye yazılır.
   Neden: iframe'in ilk about:blank belgesinden gerçek sayfaya geçişte
   tarayıcı aynı window'u yeniden kullanır. Tampermonkey script'i o boş
   belgeye enjekte ettiyse <style>'lar boş belgeyle birlikte gider, JS ise
   yaşamaya devam edip butonları yeni belgeye ekler → stilsiz butonlar
   (legacy Withdrawals popup'ında görülen). W.document her okunuşta aktif
   belgeyi verir; sandbox'ın `document`'ı eski belgede kalabiliyor. */
const styleRegistry = new Map();
function ensureStyles(doc = W.document) {
...
.gt-btn{
  ...
  color:var(--gt-text); background:#fff; border:1px solid #d9dde3; border-radius:8px;
...
.gt-btn.is-active,.gt-btn.is-active:hover{background:var(--gt-strong); border-color:var(--gt-strong); color:#fff}
    ...
    const ctx = {
        ...
        mount(anchorSel, id, build, place = 'append') {
            const install = () => {
                ...
                node.id = id;
                owned.add(node);
                ensureStyles(anchor.ownerDocument);
...
const GT = {
    ...
    bus, router, at, hotkeys, api, capture, rounds, ui, ICON, css, ensureStyles,
